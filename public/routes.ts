...
app.config(($routeProvider) => {
  var routeResolvers = {
    ...
    userSessions: (sessions, currentIdentity, auth) => {
      return auth.requireLogin().then(() => sessions.getSessionsByUser(1)
      );
    },
    allSessions: (sessions, auth) => {
      return auth.requireLogin().then(() =>  sessions.getAllSessions())
        
    },
    allUsers: (users, auth) => {
      return auth.requireLogin().then(() => users.getAllUsers());
    }
  }
